//to run
//.L myControlPlots.C+
// myControlPlots("cuttable.txt","sampleFileName.txt")
//root -l myControlPlots.C\(\"cuttable.txt\",\"sampleFileName.txt\"\)
//root -l myControlPlots.C\(\"cuttable.txt\",\"DibosonBoostedElMuSamples13TeV_2019_03_23_03h56.txt\"\)
...
public:
  Sample(const SampleInfo_t& sinfo) {
    info_ = sinfo;
    tree_ = 0;
    //    cout << "XXXXXXsample = " << sinfo.treefilename << endl;
...
#endif
    if (info_.nMCevents) {
      cout << "\nSample/xsecpblumi/nMCevents/MCnegEvent/colorcode/otherscale = " << info_.samplename << "\t" <<info_.xsecpblumi<<"\t"<<info_.nMCevents<<"\t"<<info_.MCnegEvent<<"\t"<<info_.colorcode <<"\t" << info_.otherscale << endl;
      cout << "Sample/XsecPB/lumiPBm1/(Ngen- 2*nMCnegEvent)| scale  = " << info_.samplename << "/" << info_.xsecpblumi << info_.otherscale   << "/" << (info_.nMCevents - 2*info_.MCnegEvent) << "|" <<  info_.xsecpblumi*info_.otherscale/(info_.nMCevents - 2*info_.MCnegEvent) << endl;
      histo->Scale((info_.xsecpblumi*info_.otherscale)/(info_.nMCevents - 2*info_.MCnegEvent));
   
      cout <<  endl << "!? ===" <<histo->IntegralAndError(1,histo->GetNbinsX(),tmp) << " " <<tmp<< " " << (histo->Integral(1,histo->GetNbinsX()+1)*info_.xsecpblumi*info_.otherscale)/(info_.nMCevents - 2*info_.MCnegEvent) << " " << " scaled events in window" << endl;
    }
    cout << endl;
...
void loadSamples(const char *filename,vector<Sample *>& samples)
{
  ...
  for (int i=0; !feof(fp) && fgets(line,512,fp); i++) {
    ...
    cout << "Loading sample = samplefile/xsec/xsecpblumi/ngen/" << s.samplename << " = " << s.treefilename  << "/" <<  s.xsecpblumi << "/" <<  s.nMCevents   << endl;
 ...
 // Data
  ...
  for (int ivar=0; ; ivar++) {
	...
	if (s->stackit()) {
      ...
      else if (s->name().EqualTo("DCH_WW_M500")){
	if (int(ScaleSignal) == 1)
