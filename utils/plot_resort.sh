...
if [[ $1 == 2016 || $1 == 2017 || $1 == 2018 || $1 == 0000 ]]; then
	...
	echo "Done sorting control plots, next is shape plots"

	for file in plots/$1/archive/s1_$1_*.pdf; do
		cut=$(echo $file | sed 's|^.*\([0-9]\{4\}_\)||g' | sed 's|[0-9]\+.pdf||g') # 'cut' pulls out the end of the file name which corresponds to the cutName used
		if [[ ! "${pdfs[@]}" =~ "$cut" ]]; then
			pdfs+=("$cut")
		fi
	...
	for cuts in ${pdfs[@]}; do
		current=$(find plots/$1/archive/s1_$1_$cuts*.pdf | sed "s|^.*\($cuts\)||g" | sed 's|.pdf||g' | sort -rn | head -n 1) # Current highest number for the plots using the same cut
		plot_count=$(find plots/$1/s1_$1_$cuts.pdf | wc -l) # Check if the plots/####/ folder has a plot with that particular cut (0 if no, 1 if yes, shouldn't be any other number)
		if [[ $plot_count == "0" ]]; then
			mv plots/$1/archive/s1_$1_$cuts$current.pdf plots/$1/s1_$1_$cuts.pdf
			mv plots/$1/archive/s2_$1_$cuts$current.pdf plots/$1/s2_$1_$cuts.pdf
			mv plots/$1/archive/s3_$1_$cuts$current.pdf plots/$1/s3_$1_$cuts.pdf
			mv plots/$1/archive/s4_$1_$cuts$current.pdf plots/$1/s4_$1_$cuts.pdf
		elif [[ $plot_count == "1" ]]; then
			echo ".pdf plots with cut: $cuts already exist in the plots/$1 folder, skipping these."
		else
			echo "Somehow I counted more than one .pdf per cut type, plot_sort.sh should have fixed that"
