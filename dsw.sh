...
# ---------------------------------------------------------------- End - Temporary work around zone - End ---------------------------------------------------------------------------------
    ...
    for req in $LIST; do
        ...
        if  [ "$grp" == "data" ];     then
            cat $DatasetInpFile | grep $smpl | grep $yr | grep -v \!-- | awk '{print "dataSamples.push_back( new Sample(\"'${grp}'\", \"'${smpl}'\", 1, 1, gid_data, gid_data, "'${color}'", "'${nMC}'", "'${nMCneg}'") );" }' | sed 's/_noDup//g' >> $SamplesOutfile
            ...
            ls -l skims/$skim/$yr/${smpl}*root  | awk '{print $NF}' >>  skimrqs/$skim/$yr/rqs_${req}_data.lst
        else
            if  [[ "$grp" == "WV_EWK" || "$grp" == "VBS_EWK" ]]; then # WV_EWK
                if  [ "$signal" != "ewk" ]; then 
                    cat $DatasetInpFile | grep $smpl | grep $yr | grep -v \!-- | awk '{print "bkgSamples.push_back( new Sample(\"'${grp}'\", \"'${smpl}'\", '${xsec}', 1, gid_'${grp}', 1100, '${color}','${nMC}', '${nMCneg}') );"  }'   | sed 's/\.\/data\///g' | sed 's/\.root//g'  | sed 's/W+/W/g'  | sed 's/Z+/Z/g'  >> $SamplesOutfile
                else
                    ls -l skims/$skim/$yr/${smpl}*root  | awk '{print $NF}' >>  skimrqs/$skim/$yr/rqs_${req}_sgl.lst
                    cat $DatasetInpFile | grep $smpl | grep $yr | grep -v \!-- | awk '{print "sglSamples.push_back( new Sample(\"'${grp}'\", \"'${smpl}'\", '${xsec}', 1, gid_sgl, '${sid_sgl}', '${sgl_color}','${nMC}', '${nMCneg}') );"  }'   | sed 's/\.\/data\///g' | sed 's/\.root//g'  | sed 's/W+/W/g'  | sed 's/Z+/Z/g'  >> $SamplesOutfile
                ...
                ls -l skims/$skim/$yr/${smpl}*root | grep -v "_ext" | awk '{print $NF}' >>  skimrqs/$skim/$yr/rqs_${req}_bkg.lst
    ...
    for yr in $years; do
        LISTREQ="$(ls -1 skimrqs/$skim/$yr/*bkg.lst)"
        cat $LISTREQ >  skimrqs/$skim/$yr/all_bkg.lst
    done
done
