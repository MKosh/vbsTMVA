...
# WV_EWK->VBS_EWK | Diboson->VBS_QCD | top->Top | Wjets->WJets_HT | Zjets->DYJets_HT
    ...
    for group in $Groups; do
        if [ "$group" == "Data"]; then
            ...
            TempListTop="$(cat $DatasetInpFile | grep name | grep -v EWK | grep -v QCD | grep -v WJet | grep -v DYJet | grep -v Data | awk '{print $2}' | sed 's/"//g' | sed 's/^/Top--/g')"
        elif [ "$group" == "Zjets"]; then
            TempListWjets="$(cat $DatasetInpFile | grep $group | grep -v WJets_HT | awk '{print $2}' | sed 's/"//g' | sed 's/^/Wjets--/g')"
        elif [ "$group" == "Zjets"]; then
            TempListDYjets="$(cat $DatasetInpFile | grep $group | grep -v DYJets_HT | awk '{print $2}' | sed 's/"//g' | sed 's/^/Zjets--/g')"
        fi
    done

    LISTREQ="$(echo $TempListData $TempListEWK $TempListQCD $TempListTop $TempListWjets $TempListDYjets)"
#
    sid_sgl="100"
    sid_color="910"
    #signal=higgs;
    ...
    for req in $LISTREQ; do
        ...
        if  [ "$grp" == "Data" ];     then
            ...
            cat $DataInpFile | grep $smpl | grep -v "#" | awk '{print "dataSamples.push_back( new Sample("$grp ",\t" $2 ",\t 1,\t 1,\t gid_data, gid_data,\t" $14 ",\t" $8 ",\t" $11") );" }' | sed 's/_noDup//g' >> $SamplesOutfile
